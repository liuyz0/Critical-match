...
@everywhere begin
    ...
    function sample(N, M, sigY)
        C = rand(N, M) #/ M for attractor-3-1
        muY = 1.0
        Y = randn(N, M) * sigY .+ muY
...
M = 64
sigY_span = LinRange(0,1,64) 
N = 32
num_samp = 100
num_init = 50 # important!

idxs = [(i,j) for i=1:length(sigY_span), j=1:num_samp]
idxs = reshape(idxs,:)

# build parallel function
@everywhere function parallel_sol(ii, idxs, M, N, sigY_span, num_init)
    (i,j) = idxs[ii]
    sigY = sigY_span[i]
    Ss, Rs, G, C, l, κ, δ = sample(N,M,sigY)
...
@time Fractions_coll = pmap(ii -> parallel_sol(ii, idxs, M, N, sigY_span, num_init), 1:length(idxs))
Fractions = zeros(length(sigY_span),num_samp,num_init,4)
