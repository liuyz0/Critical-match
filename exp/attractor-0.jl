...
idxs = [(i,j) for i=1:length(N_span), j=1:num_samp]
idxs = reshape(idxs,:)

# build parallel function
@everywhere function parallel_sol(ii, idxs, M, N_span, ρ, num_init)
    (i,j) = idxs[ii]
    N = N_span[i]
...
@time Fractions_coll = pmap(ii -> parallel_sol(ii, idxs, M, N_span, ρ, num_init), 1:length(idxs))
Fractions = zeros(length(N_span),num_samp,num_init,4)
